'use client';

import * as React from 'react';
import { Button } from '@/components/ui/button';
import { LogOut } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useLocale } from 'next-intl';
import { toast } from 'sonner';
import { clientCleanup } from '@/lib/client-cleanup';
import { useLogoutMutiation } from '@/queries/use-auth';
import { handleErrorApi } from '@/lib/utils';

type Props = React.ComponentProps<typeof Button> & {
    withConfirm?: boolean;
    onLoggedOut?: () => void;
};

export default function LogoutButton({
    withConfirm = false,
    onLoggedOut,
    className,
    ...btnProps
}: Props) {
    const router = useRouter();
    const locale = useLocale();
    const [loading, setLoading] = React.useState(false);
    const logoutMutation = useLogoutMutiation();

    const handleLogout = async () => {
        if (withConfirm && !window.confirm('Bạn chắc chắn muốn đăng xuất?')) return;
        setLoading(true);
        if (logoutMutation.isPending) return; // Prevent multiple submissions
        try {
            await logoutMutation.mutateAsync();     // xoá cookie HttpOnly ở server + gọi BE /v1/logout
            clientCleanup();                   // xoá storage & cookies non-HttpOnly ở client
            toast.success('Logged out');
            onLoggedOut?.();
            router.push(`/${locale}/login`);
            // refresh để server/middleware đọc trạng thái mới
            router.refresh();
        } catch (e: any) {
            handleErrorApi({
                error: e,
            });
        } finally {
            setLoading(false);
        }
    };

    return (
        <Button
            variant="ghost"
            size="icon"
            onClick={handleLogout}
            disabled={loading}
            className={`h-9 w-9 text-destructive hover:text-destructive ${className ?? ''}`}
            {...btnProps}
        >
            <LogOut className="h-4 w-4" />
        </Button>
    );
}
